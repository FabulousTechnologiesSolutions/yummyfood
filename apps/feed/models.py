...
class FeedWatchOutcome(models.TextChoices):
    SKIP = 'skip', 'Skip'
    WATCH = 'watch', 'Watch'
    COMPLETE = 'complete', 'Complete'


class FeedViewerState(models.Model):
    """Per-viewer rotation offsets for Feed (auth user XOR guest ip_hash)."""
    ...
    def __str__(self):
        if self.user_id:
            return f'FeedViewerState<user={self.user_id}>'
        return f'FeedViewerState<ip={self.ip_hash}>'


class FeedImpression(models.Model):
    """Per-viewer serve + watch history for Feed ranking."""
    ...
    watched_ms = models.PositiveIntegerField(null=True, blank=True)
    duration_ms = models.PositiveIntegerField(null=True, blank=True)
    outcome = models.CharField(
        max_length=16,
        choices=FeedWatchOutcome.choices,
        blank=True,
        default='',
    )
    ...
    def __str__(self):
        ...
        return f'FeedImpression<{target} serves={self.serve_count} watched={self.watched_ms}>'
